...
export default function SessionSidebar({
                                           salas = [], // <--- FIX 1: Valor padrão aqui
                                           selectedRoomId, onSelectRoom,
                                           sessions = [], // <--- FIX 2: Valor padrão aqui também
                                           onRemoveSession, onDuplicateSession,
                                           newSession, setNewSession, onAddManual, onSelectFullDay
                                       }: SessionSidebarProps) {

    // --- FIX 3: Variáveis de segurança extra ---
    ...
    return (
        <div className="w-full lg:w-1/3 flex flex-col gap-4 h-full">
            ...
            <div className={`p-4 rounded-xl shadow-md border-2 transition-colors ${!selectedRoomId ? 'bg-red-50 border-red-300 animate-pulse' : 'bg-slate-900 border-slate-900 text-white'}`}>
                ...
                >
                    {/* Alterado de salas para safeSalas */}
                    {safeSalas.length === 0 && <option value="" disabled>Sem salas disponíveis</option>}
                    {safeSalas.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                </select>
            </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar border-t pt-4 dark:border-slate-800">
                <h4 className="text-xs font-bold text-slate-400 uppercase mb-2">Sessões ({safeSessions.length})</h4>
